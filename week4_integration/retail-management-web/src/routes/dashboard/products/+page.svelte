<script lang="ts">
    ...
    interface Product {
        product_id: string;
        product_name: string;
        purchase_price: string; // JSON dari Go string ("8250.75")
    ...
    interface Category {
        category_id: string;
        category_name: string; // Sesuaikan dengan response backend
    }

    interface Supplier {
        supplier_id: string;
        supplier_name: string; // Sesuaikan dengan response backend
    }
    ...
    // --- STATE MODAL ADD & UPDATE ---
    let showAddModal = $state(false);
    let showUpdateModal = $state(false);

    // Form Variables (Add/Update)
    // Kita gunakan tipe string untuk number input agar mudah handling desimal
    let formId = $state('');
    let formName = $state('');
    ...
    // --- ON MOUNT ---
    onMount(async () => {
        // Load semua data master yang dibutuhkan secara paralel
        await Promise.all([
            loadProfile(),
            loadCategories(),
            loadSuppliers(),
    ...
    async function loadProfile() {
        try {
            const response = await fetchFromGo('/auth/me'); // Sesuai permintaan (auth/me)
    ...
    function openAddModal() {
        // Reset Form
        formName = ''; formPurchase = ''; formSelling = ''; formStock = ''; 
        formCatId = ''; formSupId = '';
        showAddModal = true;
    }
    ...
    // --- 2. DETAILS PRODUCT (GET /products/:id) ---
    async function handleDetail(id: string) {
        try {
            // Fetch detail realtime dari backend
            const res = await fetchFromGo(`/products/${id}`);
            const data = res.data || res;
            
            detailProduct = data;
            // Map ID ke Nama untuk tampilan
    ...
    async function handleUpdateSubmit(e: Event) {
        ...
        try {
            ...
            const payload = {
                ...
                category_id: formCatId,
                supplier_id: formSupId
                // Stock quantity biasanya punya endpoint khusus (restock), 
                // tapi jika mau diupdate disini juga bisa ditambahkan.
            };
...
<style>
    /* CSS BAWAAN ANDA (Tanpa perubahan mayor, hanya penyesuaian display popup) */
    * { box-sizing: border-box; }
    /* body selector dihapus karena di Svelte scope komponen, gunakan :global(body) jika perlu, 
       tapi disini kita ikut style parent layout */
...
>>>>>>> f634aa1a007b005500f82cb928b6c467d0bcdef0
    ...
    .details, .delete, .update { padding: 8px 15px; border: none; border-radius: 20px; margin-left: 5px; cursor: pointer; font-size: 14px; transition: scale 0.1s ease; }
    .details { background-color: rgb(62, 48, 255); color: white; }
    ...
    .details:hover{ scale: 110%; }

    /* POPUP DETAIL CUSTOM */
    /* Saya hapus 'display: none' agar bisa dikontrol oleh Svelte {#if} */
    .popup-overlay {
    ...
    /* .gambarpopup dihapus sesuai request */

    .descpopup p { margin: 5px 0; font-size: 16px; border-bottom: 1px dashed #eee; padding-bottom: 5px;}

    .closepopup { 
        background-color: transparent; border: none; font-size: 20px; cursor: pointer; font-weight: bold; transition: scale 0.2s ease, color 0.1s ease;
    }
    ...
    /* Style form untuk Modal Add/Update */
